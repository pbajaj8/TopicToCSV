function [x_ekf_bro, AB, Pt, e] = ekf_with_broydenUpdate(filepath, Q)
    ...
    for i = 2:n_row
        ...
        if (rem(i,10) == 0)
            ...
            x_ekf_bro = [x_ekf_bro; transpose(Xtt)];
            
            e = [e; transpose(Zt - Xt_hat)];

            sprintf("Measuremnt update");
            ...
            if (measurementCounter < 2)
                ...
                %Broyden's Update
                df = xb_output2 - xb_output1; 
                dx = xb_inpute2 - xb_input1;
                
                sprintf('dx_norm_sq: %f ', (norm(dx)^2));
                lol = (df - AB * dx) * transpose(dx) / (norm(dx)^2);
                lol_inter = [[0 0 0; 0 0 0 ; 0 0 0],lol(:,4:6)]; 
                %AB = AB +  lol_inter;  
                AB = AB + lol_inter; 
                sprintf("Broydans Update");

                % Update for next cycle 
                xb_input1 = xb_inpute2; 
                xb_output1 = xb_output2; 
            end 
            ...
            x_ekf_bro = [x_ekf_bro; transpose(Xt_hat)];
        ...
        %U = [0; (-1e-3)*table2array(T(i:i, "ZaberVel")); 0];
